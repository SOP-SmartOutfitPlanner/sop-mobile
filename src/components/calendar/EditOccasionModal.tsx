...
import { EditUserOccasionRequest, UserOccasion } from "../../types/userOccasion";
import { CalendarUserOccasion } from "../../types/calendar";

interface EditOccasionModalProps {
  visible: boolean;
  occasion: CalendarUserOccasion | UserOccasion | null;
  onClose: () => void;
  onSuccess: () => void;
  onUpdateOccasion?: (id: number, data: Partial<EditUserOccasionRequest>) => Promise<UserOccasion | null>;
}

export const EditOccasionModal: React.FC<EditOccasionModalProps> = ({
  visible,
  occasion,
  onClose,
  onSuccess,
  ...
  useEffect(() => {
    if (visible && occasion) {
      fetchOccasions();
      // Populate form with existing occasion data
      setName(occasion.name || "");
      setDescription(occasion.description || "");
      setStartTime(occasion.startTime || "");
      setEndTime(occasion.endTime || "");
      setSelectedOccasionId(occasion.occasionId || null);
      setErrors({});
    } else if (!visible) {
  ...
  const handleSubmit = async () => {
    if (!validate() || !occasion) return;

    setLoading(true);
    try {
      const updateData: Partial<EditUserOccasionRequest> = {
        name: name.trim(),
        description: description.trim() || undefined,
        occasionId: selectedOccasionId || undefined,
        startTime: startTime || undefined,
        endTime: endTime || undefined,
        weatherSnapshot: "", // Can be enhanced later
      };

      let updatedOccasion: UserOccasion | null = null;
      
      if (onUpdateOccasion) {
        // Use hook function (with notification)
        updatedOccasion = await onUpdateOccasion(occasion.id, updateData);
        ...
        const response = await CalenderAPI.updateUserOccasion(occasion.id, updateData);
        if (response.statusCode === 200 && response.data) {
          updatedOccasion = response.data;
          onSuccess();
          onClose();
        }
      }
      ...
      setErrors({ name: error.message || "Failed to update occasion" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const selectedDate = occasion.dateOccasion ? new Date(occasion.dateOccasion) : null;
      ...
      <SafeAreaView style={styles.container} edges={["top", "bottom"]}>
        ...
        <View style={styles.header}>
          <View style={styles.headerContent}>
            <Text style={styles.headerTitle}>Edit Occasion</Text>
              ...
              >
                {occasions.map((occ) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.occasionChip,
                      selectedOccasionId === occ.id && styles.occasionChipSelected,
                  ...
                  >
                    <Text
                      style={[
                        ...
                        selectedOccasionId === occ.id && styles.occasionChipTextSelected,
                      ]}
                    >
                      {occ.name}
                    </Text>
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <Text style={styles.submitButtonText}>Update Occasion</Text>
              </>
            )}
          </TouchableOpacity>
        </View>
